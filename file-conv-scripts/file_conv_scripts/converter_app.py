...
from file_conv_framework.base_converter import ResolvedInputFile, BaseConverter
...
class BaseConverterApp:
    ...
    def __init__(self, input_file_path, input_file_type=None, output_file_path=None, output_file_type=None):
        ...
        self.input_file = ResolvedInputFile(input_file_path, file_type=input_file_type, add_suffix=False, read_content=True)
        if not output_file_path:
            output_file_path = str(Path(input_file_path).with_suffix(''))
        self.output_file = ResolvedInputFile(output_file_path, file_type=output_file_type, add_suffix=True, read_content=False)
    ...
    def add_converter_pair(self, converter_class: Type[BaseConverter]):
        ...
        self._dict_converters[(converter_class.get_input_type(), converter_class.get_output_type())] = converter_class
    ...
    def run(self):
        if self.output_file:
            converter_class = self._dict_converters.get((self.input_file.file_type, self.output_file.file_type))
            if converter_class:
